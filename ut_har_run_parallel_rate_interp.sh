#!/usr/bin/env bash
## nohup ./ut_har_run_parallel_rate_interp.sh > rate_interp_run.log 2>&1 &
#####################################################################
# run_grid_search.sh
# 终极版并行脚本，用于大规模网格搜索实验。
...
# 2. 可用GPU
GPU_LIST=(4 5 6 7 )
# 3. 基础实验名称 (将作为所有子实验目录的前缀,amp对应energy，rate_interp对应mask)
BASE_EXP_NAME="amp_rate_interp_$(date +%Y%m%d_%H%M)"
use_energy_input=0      # 1: 使用能量信息 (True)
use_mask_0=0            # 0: 不使用 mask_0 (False)

# 4. 【新增循环】采样方法 (对应 --sample_method)
# 如果想跑多种方法，可以在括号里加：uniform_nearest equidistant gaussian poisson
SAMPLE_METHODS=(gaussian poisson)
...
for s_method in "${SAMPLE_METHODS[@]}"; do
    for rate in "${SAMPLE_RATES[@]}"; do
        for i_method in "${INTERPOLATION_METHODS[@]}"; do
            for model in "${MODELS[@]}"; do
                # 【修改】将 sample_method 加入任务字符串
                # 格式: sample_method:sample_rate:interpolation_method:model_name
